...
- Modal dialog for photo selection with preview
- Accept multiple file inputs via:
  - File picker (drag & drop support)
  - Copy/paste images (Ctrl+V / Cmd+V)
- For each selected photo:
  - Show thumbnail preview
  - Individual caption input field
  - Remove button to deselect
- Post-level content field (optional post text)
- Visibility selector (private/friends/public)
- Submit button to create gallery Thing

**Frontend Photo Upload Flow**:
1. User opens upload modal
2. User selects photos (drag-drop, file picker, or paste)
3. Each photo shows thumbnail + caption input field
4. User can remove individual photos
5. User enters optional post content and visibility level
6. Submit creates single gallery Thing with all photos in order
